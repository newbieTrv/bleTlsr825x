...
#define SPP_RXFIFO_NUM		4

#define SPP_TXFIFO_SIZE		4//252//72
#define SPP_TXFIFO_NUM		2


MYFIFO_INIT(spp_rx_fifo, (SPP_RXFIFO_SIZE), SPP_RXFIFO_NUM);//size ����4�ֽڶ���   numΪ2�ı���
MYFIFO_INIT(spp_tx_fifo, (SPP_TXFIFO_SIZE), SPP_TXFIFO_NUM);//
...
u8	tbl_advData[] = {
 ...
 u8	tbl_scanRsp [] = {
		 0x07, 0x09, 'l', 'a', 'n', '-', 'd', 'd',	//scan name " tmodul"
	};
...
extern unsigned int uart_tx_buffer_data_len();

//#define UART_TX_BUSY			( (spp_tx_fifo.rptr != spp_tx_fifo.wptr) || uart_tx_is_busy() )
#define UART_TX_BUSY            ( uart_tx_buffer_data_len() || uart_tx_is_busy())
...
#else
	if(blt_state==BLS_LINK_STATE_IDLE)
	{//idle ״̬
		//û�й㲥�������¼�����ʱ����Ҫ�Լ���ʱ����
		if(rtc_timer_vaild)
		{
			tuya_timer_start(TIMER_RTC,15000);
		}
		else
		{
			tuya_timer_delete(TIMER_RTC);
		}

		if(blt_soft_timer_get_timer_num()==0)
		{//ֹͣ�㲥����û�ж�ʱ����ʱ���������˯��
			if (!app_module_busy() && !tick_wakeup)
			{
		...
		{//ֹͣ�㲥�����ж�ʱ����ʱ������suspend mode,��ʱ������
			if (!app_module_busy() && !tick_wakeup)
			{
                #if (PM_DEEPSLEEP_RETENTION_ENABLE)
					cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM_LOW32K,PM_WAKEUP_TIMER|PM_WAKEUP_PAD,blt_soft_timer_get_next_timeout());
				#else
					cpu_sleep_wakeup(SUSPEND_MODE,PM_WAKEUP_TIMER|PM_WAKEUP_PAD,blt_soft_timer_get_next_timeout());
				#endif
			}
...
static const char auth_key_test[] = "9auEGvyaJsBPTBaURlbKxuwuD9bClxFc";
static const char h_id_test[] = "1462859897706602496";
static const char device_id_test[] = "dce178ce7028f4ac";
...
////////////////// BLE stack initialization ////////////////////////////////////
	...
	tuya_ble_get_mac(mac_public);//��ȡmac
...
_attribute_ram_code_ void user_init_deepRetn(void)
{
	...
	//cpu_set_gpio_wakeup (GPIO_WAKEUP_MODULE, Level_High, 1);  // pad high wakeup deepsleep
	//GPIO_WAKEUP_MODULE_LOW;
	//tuya_log_d("825x demo deepRetn init ok");
	//DBG_CHN0_HIGH;   //debug
#if (BLE_MODULE_INDICATE_DATA_TO_MCU)
	GPIO_WAKEUP_MCU_LOW;
#endif
...
void main_loop (void)
{
	static u32 soft_timer_last_tick=0;
	static u32 rtc_timer_last_tick=0;
	static u32 ms_timer_last_tick=0;
	...
	if(clock_time_exceed(soft_timer_last_tick,500))
	{
