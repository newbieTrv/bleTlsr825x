...
//	}
	if(len > 0)
	{
		tuya_ble_gatt_receive_data(&(p->value),len);
	}
	return 0;
}
...
//tuya
u8 TuyaSppServiceUUID[2]	          = {0x10,0x19};//tuya_SPP_UUID_SERVICE;
u8 TuyaSppDataServer2ClientUUID[2]    = {0x10,0x2B};//tuya_SPP_DATA_SERVER2CLIENT;
u8 TuyaSppDataClient2ServerUUID[2]    = {0x11,0x2B};//tuya_SPP_DATA_CLIENT2SERVER;

// Spp data from Server to Client characteristic variables
static u8 TuyaSppDataServer2ClientProp = CHAR_PROP_READ | CHAR_PROP_NOTIFY;
u8  TuyaSppDataServer2ClientData[ATT_MTU_SIZE - 3];
static u8 TuyaSppDataServer2ClientDataCCC[2] = {0};

// Spp data from Client to Server characteristic variables
//CHAR_PROP_WRITE: Need response from slave, low transmission speed
static u8 TuyaSppDataClient2ServerProp = CHAR_PROP_READ | CHAR_PROP_WRITE|CHAR_PROP_WRITE_WITHOUT_RSP;
u8  TuyaSppDataClient2ServerData[ATT_MTU_SIZE - 3];
...
{
	bls_att_setAttributeTable ((u8 *)tuya_Attributes);
